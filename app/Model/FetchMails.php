...
class FetchMails extends Model
{
    protected $marubox = '';
    ...
    public function get_email_header($mid = null) // Get Header info
    {
        ...
        $data['mail_details'] = array();
        if (strtolower($sender->mailbox) != 'mailer-daemon' && strtolower($sender->mailbox) != 'postmaster') {
            $data['mail_details'] = array(
                'created_at' => date("Y-m-d H:i:s", $mail_header->udate),

                'updated_at' => date("Y-m-d H:i:s", $mail_header->udate),
                ...
                'subject' => strtoupper(iconv_mime_decode($mail_header->subject, 0, "utf-8")),
                //  'to' => strtolower($mail_header->toaddress),
                // 'full_header' => serialize($mail_header)
                'full_header' => json_decode(json_encode($mail_header)),
                // 'full_header' => json_encode($mail_header),

            );
        }
        // echo "<pre>";
        // $header = json_decode(json_encode($mail_header));
        // $header = json_decode($header);

        // print_r($data['mail_details']['full_header']->message_id);
        // echo "<pre>";
        // print_r($data['mail_details']);
        // exit;
        return $data;
    }
    ...
    //     // exit;
    //     if ($struct->parts) {
    //         // if (array_key_exists('parts', $struct)) {
    //         // echo "fred";
    //         // exit;
